...
mcp = FastMCP(
    ...
    instructions=(
        ...
        # A grower's agent reads this first, and it used to open straight onto
        # the OPERATOR's setup — Authority registration and BTCPay secrets —
        # which is nothing a grower does. Their path comes first now.
        "## First connection, for a grower's agent\n"
        "1. Ask the grower for their Nostr npub (npub1…). Never ask for an nsec.\n"
        "2. goodearth_request_npub_proof(patron_npub=…) sends them a Nostr DM. "
        "Tell them to reply to it from their Nostr client, then call "
        "goodearth_receive_npub_proof(patron_npub=…, dpop_token=…) once. Pass "
        "npub and dpop_token on every paid call after that.\n"
        "3. goodearth_check_balance. If it is empty, goodearth_purchase_credits "
        "returns a Lightning invoice for the grower to pay, and "
        "goodearth_check_payment confirms it.\n"
        "4. goodearth_block_list — their saved ground, or a worked example.\n"
        "Needing none of that: goodearth_service_status and the "
        "goodearth_oracle_* tools. goodearth_check_price previews a fare. The "
        f"same tools drive the web app at {SITE}.\n\n"
        "## Operator onboarding\n"
        "For the operator of this service, not a grower. "
